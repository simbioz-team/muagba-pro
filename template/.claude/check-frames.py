...
# Маркеры-слова. «заполни» в свободном тексте — обычное слово («заполните
# форму»), поэтому здесь его нет: в заготовках оно всегда стоит внутри
# угловых скобок, а те ловятся отдельно. Остальные три однозначны.
WORD_SLOTS = ("НАЗВАНИЕ ПРОЕКТА", "TODO", "FIXME")
